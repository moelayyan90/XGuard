...
if (initialize.body.result?.serverInfo?.name !== "xguard-agent-execution-gateway" || initialize.body.result?.serverInfo?.version !== VERSION) fail("MCP initialize has stale identity");
...
if (names.size !== 8) fail("Internal tools leaked into the primary catalog");
...
if (!developers.ok || !developersText.includes("/v1/providers/operations") || !developersText.includes("stable idempotency key")) fail("Developer quickstart is missing or incomplete");
if (!developersText.includes("/v1/secretless/call")) fail("Developer page does not lead to a real customer purchase");
