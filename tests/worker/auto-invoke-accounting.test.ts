...
describe("auto-invoke accounting finalization", () => {
  it("earns after a provider 2xx when accounting succeeds", async () => {
  ...
  it("releases instead of forcing a provider retry when earning fails before transition", async () => {
    const merchantId = await fundedMerchant(1_000);
  ...
  it("reports released for a failed provider call whose reservation is returned", async () => {
    const merchantId = await fundedMerchant(1_000);
