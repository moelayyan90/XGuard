...
describe("manual deployment script production target", () => {
  it("is hard-locked to xguard-mainnet and the universal entrypoint", async () => {
    ...
    expect(source).toContain("src/universal-mainnet.ts");
    expect(source).toContain("npm run verify:mainnet-release");
    expect(source).toContain("/.well-known/xguard/protocols.json");
