...
(() => {
  ...
  const TASK_PATTERNS = [
    ["authentication", /\b(log ?in|sign ?in|sign ?up|register|account|verify|verification|password|otp|two[- ]?factor|2fa|mfa)\b|تسجيل الدخول|إنشاء حساب|انشاء حساب|تحقق|رمز التحقق|كلمة المرور/i],
    ["booking", /\b(book|booking|reserve|reservation|appointment|flight|hotel|ticket|schedule)\b|حجز|موعد|رحلة|فندق|تذكرة/i],
    ["application", /\b(apply|application|submit application|request|enroll|registration form|claim)\b|تقديم طلب|طلب جديد|تسجيل|مطالبة/i],
    ["upload", /\b(upload|attach|attachment|document|file|photo|image|resume|cv)\b|رفع ملف|إرفاق|ارفاق|مستند|وثيقة|صورة/i],
    ["commerce", /\b(checkout|buy|purchase|order|cart|payment|pay|subscribe|invoice|billing)\b|شراء|طلب|سلة|دفع|فاتورة|اشتراك/i],
    ["transfer", /\b(transfer|beneficiary|recipient|send money|remit|withdraw|deposit)\b|تحويل|مستفيد|إرسال المال|ارسال المال|سحب|إيداع/i],
    ["settings", /\b(settings|preferences|profile|configure|configuration|update|save changes|security settings)\b|إعدادات|اعدادات|تفضيلات|الملف الشخصي|حفظ التغييرات/i],
    ["support", /\b(support|help request|ticket|refund|return|dispute|complaint|contact us)\b|دعم|تذكرة دعم|استرداد|إرجاع|نزاع|شكوى|اتصل بنا/i],
  ];
  const FAILURE_WORDS = /\b(error|failed|failure|invalid|required|missing|declined|rejected|denied|unable|cannot|couldn't|expired|blocked|unavailable|not available|try again|something went wrong|timeout|timed out|too many requests|rate limit)\b|خطأ|فشل|غير صالح|مطلوب|مفقود|مرفوض|تم الرفض|غير متاح|حاول مرة أخرى|انتهت الصلاحية|محظور/i;
  const HUMAN_WORDS = /\b(captcha|prove you are human|human verification|verification code|one[- ]?time code|otp|mfa|2fa|two[- ]?factor|security key|approve on your phone|scan qr|login required|sign in required)\b|كابتشا|تحقق أنك إنسان|رمز التحقق|المصادقة الثنائية|سجّل الدخول|سجل الدخول/i;
  const RETRY_WORDS = /\b(try again|retry|reload|temporary|temporarily|timeout|timed out|too many requests|rate limit|network error)\b|حاول مرة أخرى|أعد المحاولة|اعادة المحاولة|مؤقت|انتهت المهلة/i;
  const UNAVAILABLE_WORDS = /\b(sold out|fully booked|out of stock|unavailable|not available|no availability|no slots)\b|نفد|ممتلئ|لا يوجد حجز|غير متاح|لا توجد مواعيد/i;
  ...
  async function scan() {
    ...
    if (diagnosis.status === "ATTENTION" || diagnosis.status === "HUMAN_REQUIRED") {
      panelOpen = true;
    }
    render();
  }
  ...
  function detectTaskSurface() {
    const pageText = `${location.pathname} ${document.title || ""} ${visibleHeadings()} ${visibleActions()}`.slice(0, 12000);
    const forms = Array.from(document.forms).slice(0, 60);
    const actionable = Array.from(document.querySelectorAll("button,[role='button'],input[type='submit'],input[type='button'],a[href]")).filter(isVisible).slice(0, 240);
    const requiredFields = Array.from(document.querySelectorAll("input[required],select[required],textarea[required]")).filter(isVisible);
    ...
    for (const [category, pattern] of TASK_PATTERNS) {
      ...
      for (const form of forms.slice(0, 15)) {
        const text = `${form.getAttribute("action") || ""} ${form.textContent || ""}`.slice(0, 1800);
        if (pattern.test(text)) score += 2;
      }
      scores.set(category, score);
    }
    if (fileInputs.length) scores.set("upload", (scores.get("upload") || 0) + 4);
    if (forms.length) scores.set("generic", 2 + Math.min(forms.length, 2));
    if (requiredFields.length >= 2) scores.set("generic", (scores.get("generic") || 0) + 2);
  ...
  function diagnosePage(task) {
    ...
    const invalid = Array.from(document.querySelectorAll('[aria-invalid="true"],input:invalid,select:invalid,textarea:invalid')).filter(isVisible);
    const required = Array.from(document.querySelectorAll("input[required],select[required],textarea[required]")).filter(isVisible);
    const incomplete = required.filter(isIncomplete);
    const text = alerts.map((node) => compactText(node.textContent || "")).filter(Boolean).join(" | ").slice(0, 5000);
    const bodySample = compactText((document.body?.innerText || "").slice(0, 18000));
    ...
    if (humanRequired) issues.push("تحتاج هذه الخطوة تدخلاً بشريًا أو تحقق هوية.");
    if (invalid.length) issues.push(`يوجد ${invalid.length} حقل غير صالح.`);
    else if (incomplete.length && hasFailure) issues.push(`يوجد ${incomplete.length} حقل مطلوب غير مكتمل.`);
    if (unavailable) issues.push("الخيار الحالي غير متاح ويحتاج تغيير اختيار أو موعد أو مخزون.");
    if (retryable) issues.push("العطل يبدو مؤقتًا ويمكن إعادة المحاولة بعد مراجعة الحالة.");
    if (hasFailure && !issues.length) issues.push(text ? truncate(text, 180) : "اكتشف XGuard رسالة فشل في العملية الحالية.");

    let status = task ? "READY" : "IDLE";
    if (hasFailure || invalid.length || (task && incomplete.length && actionLooksBlocked())) status = "ATTENTION";
    if (humanRequired) status = "HUMAN_REQUIRED";

    let nextAction = "راقب العملية واحفظ نقطة رجوع قبل الخطوات الحساسة.";
    if (invalid[0]) nextAction = "انتقل إلى أول حقل غير صالح وصححه قبل المتابعة.";
    else if (incomplete[0]) nextAction = "أكمل أول حقل مطلوب غير مكتمل.";
    else if (humanRequired) nextAction = "أكمل التحقق البشري ثم استأنف من نفس الصفحة.";
    else if (unavailable) nextAction = "غيّر الاختيار الحالي بدل تكرار نفس الطلب.";
    else if (retryable) nextAction = "راجع الرسالة ثم أعد المحاولة من نفس الخطوة.";
    else if (hasFailure) nextAction = "افتح موضع الخطأ واقرأ الرسالة قبل إعادة التنفيذ.";
    ...
    for (const selector of selectors) {
      try {
        document.querySelectorAll(selector).forEach((node) => {
          if (isVisible(node) && compactText(node.textContent || "").length) found.add(node);
        });
      } catch {
  ...
  function visibleActions() {
    return Array.from(document.querySelectorAll("button,[role='button'],input[type='submit'],a[href]"))
      .filter(isVisible)
      .slice(0, 100)
      .map(labelOf)
      .filter(Boolean)
  ...
  function findPrimaryAction(candidates) {
    const strong = /\b(submit|continue|next|save|confirm|apply|book|reserve|upload|send|pay|purchase|checkout|login|sign in|register|create|update|finish)\b|إرسال|ارسال|متابعة|التالي|حفظ|تأكيد|تقديم|حجز|رفع|دفع|شراء|تسجيل|إنشاء|انشاء|تحديث|إنهاء|انهاء/i;
    return candidates.find((node) => strong.test(labelOf(node))) || candidates.find((node) => node.tagName === "BUTTON" || node.getAttribute("role") === "button") || null;
  }

  function actionLooksBlocked() {
    const action = findPrimaryAction(Array.from(document.querySelectorAll("button,[role='button'],input[type='submit']")).filter(isVisible));
    return Boolean(action && (action.disabled || action.getAttribute("aria-disabled") === "true"));
  }
  ...
  function labelOf(node) {
    return compactText(`${node.value || ""} ${node.textContent || ""} ${node.getAttribute?.("aria-label") || ""}`);
  }
    ...
    try {
      ...
      if (value && typeof value === "object" && !Array.isArray(value)) {
        return {
          ...
          activeByOrigin: value.activeByOrigin && typeof value.activeByOrigin === "object" ? value.activeByOrigin : {},
        };
      }
    } catch {
  ...
  async function saveCheckpoint() {
    ...
    model.stored.checkpoints = model.stored.checkpoints.slice(0, MAX_CHECKPOINTS);
  ...
  async function markComplete() {
    ...
    if (!active) return;
    const item = model.stored.checkpoints.find((entry) => entry.id === active.id);
    if (item) {
  ...
  function focusRecoveryTarget() {
    const invalid = Array.from(document.querySelectorAll('[aria-invalid="true"],input:invalid,select:invalid,textarea:invalid')).filter(isVisible);
    const incomplete = Array.from(document.querySelectorAll("input[required],select[required],textarea[required]")).filter((node) => isVisible(node) && isIncomplete(node));
    const problems = collectProblemNodes();
    const target = invalid[0] || incomplete[0] || problems[0] || findPrimaryAction(Array.from(document.querySelectorAll("button,[role='button'],input[type='submit']")).filter(isVisible));
    if (!target) return;
  ...
  function ensureHost() {
    ...
    host.style.cssText = "all:initial;position:fixed;top:14px;left:50%;transform:translateX(-50%);z-index:2147483647;width:min(620px,calc(100vw - 24px));pointer-events:none";
  ...
  function render() {
    ...
    const diagnosis = model?.diagnosis || { status: "IDLE", issues: [], nextAction: "" };
    const active = model?.active;
    const statusText = diagnosis.status === "HUMAN_REQUIRED" ? "تدخل مطلوب" : diagnosis.status === "ATTENTION" ? "تعطل مكتشف" : active ? "نقطة رجوع محفوظة" : "المهمة تحت المراقبة";
    const statusClass = diagnosis.status === "HUMAN_REQUIRED" ? "human" : diagnosis.status === "ATTENTION" ? "attention" : "ready";
    const issueHtml = diagnosis.issues.length ? diagnosis.issues.map((issue) => `<li>${escapeHtml(issue)}</li>`).join("") : `<li>لم يكتشف XGuard عطلًا حاليًا.</li>`;
    ...
    bind("toggle", () => { panelOpen = !panelOpen; render(); });
  ...
  observer.observe(document.documentElement, { childList: true, subtree: true, attributes: true, attributeFilter: ["aria-invalid", "aria-disabled", "disabled", "class"] });
  document.addEventListener("visibilitychange", scheduleScan, { passive: true });
  document.addEventListener("input", () => scheduleScan(320), { capture: true, passive: true });
  document.addEventListener("submit", () => scheduleScan(700), { capture: true, passive: true });
  document.addEventListener("click", () => scheduleScan(520), { capture: true, passive: true });
