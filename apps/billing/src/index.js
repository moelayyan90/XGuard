...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/healthz") { const packageInfo = publicPackage(env); return json({ status: env.LEMONSQUEEZY_WEBHOOK_SECRET ? "ready" : "not_ready", service: "XGuard — Agent Execution Gateway billing", version: VERSION, webhook_signature: env.LEMONSQUEEZY_WEBHOOK_SECRET ? "configured" : "missing", package: packageInfo }, env.LEMONSQUEEZY_WEBHOOK_SECRET ? 200 : 503, responseHeaders); }
