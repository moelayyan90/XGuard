...
function connectPage() {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Connect XGuard 5.1.0 — MCP for Copilot, Claude, Cursor, VS Code and Cline</title><meta name="description" content="Install the XGuard x402 Routing Surface remote MCP in GitHub Copilot, Claude Code, Cursor, VS Code or Cline."><link rel="canonical" href="https://xguardgate.com/connect"><style>body{margin:0;background:#07090d;color:#f5f7fb;font-family:Inter,system-ui,sans-serif}.w{width:min(900px,calc(100% - 32px));margin:auto;padding:64px 0}h1{font-size:clamp(42px,7vw,72px);letter-spacing:-.055em;margin:16px 0}.muted{color:#97a4b7;line-height:1.7}.badge{display:inline-block;border:1px solid #35578a;border-radius:99px;padding:7px 11px;color:#a9c6ff;font-size:12px}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px;margin-top:32px}.c{border:1px solid #202938;border-radius:16px;background:#0d1118;padding:22px}.c h2{margin-top:0}.btn{display:inline-block;margin-top:10px;padding:11px 14px;border-radius:10px;background:#4f8cff;color:white;text-decoration:none;font-weight:800}code,pre{font-family:ui-monospace,SFMono-Regular,Consolas,monospace}pre{white-space:pre-wrap;background:#080b10;border:1px solid #202938;padding:13px;border-radius:10px;color:#cfe0ff}.foot{margin-top:35px;color:#738095;font-size:12px}@media(max-width:680px){.grid{grid-template-columns:1fr}}</style></head><body><main class="w"><span class="badge">XGuard v5.1.0 · Remote Streamable HTTP MCP</span><h1>Connect XGuard.</h1><p class="muted">Canonical endpoint: <strong>https://api.xguardgate.com/mcp</strong>. Ten live tools expose x402 facilitator identity, route selection, Bazaar discovery, transaction inspection, safety checks, supported payment kinds, health and receipt lookup.</p><div class="grid"><section class="c"><h2>GitHub Copilot CLI</h2><pre>copilot plugin install moelayyan90/XGuard</pre><p class="muted">Or add the repository as a plugin marketplace.</p></section><section class="c"><h2>Claude Code</h2><pre>claude mcp add --transport http xguard https://api.xguardgate.com/mcp</pre></section><section class="c"><h2>Cursor</h2><p class="muted">One-click MCP installation.</p><a class="btn" href="cursor://anysphere.cursor-deeplink/mcp/install?name=xguard&config=eyJ4Z3VhcmQiOnsidXJsIjoiaHR0cHM6Ly9hcGkueGd1YXJkZ2F0ZS5jb20vbWNwIn19">Add to Cursor</a></section><section class="c"><h2>VS Code / Copilot</h2><p class="muted">Install the remote HTTP MCP configuration.</p><a class="btn" href="vscode:mcp/install?%7B%22name%22%3A%22xguard%22%2C%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.xguardgate.com%2Fmcp%22%7D">Install in VS Code</a></section><section class="c"><h2>Cline</h2><p class="muted">Add a Remote MCP Server named <code>xguard</code> with URL:</p><pre>https://api.xguardgate.com/mcp</pre></section><section class="c"><h2>Machine discovery</h2><p><a class="btn" href="https://api.xguardgate.com/.well-known/mcp/server.json">MCP metadata</a></p><p><a class="btn" href="https://api.xguardgate.com/openapi.json">OpenAPI</a></p></section></div><p class="foot">Non-custodial x402 v2 facilitator. XGuard does not claim downstream facilitator signer or settlement addresses as its own.</p></main></body></html>`;
}
...
function staticDiscovery(request) {
  ...
  if (url.pathname === "/.well-known/security.txt") {
    const body = ["Contact: mailto:mo.elayyan2023@gmail.com", "Canonical: https://xguardgate.com/.well-known/security.txt", "Canonical: https://api.xguardgate.com/.well-known/security.txt", "Preferred-Languages: en, ar", "Policy: https://github.com/moelayyan90/XGuard/security", "Expires: 2027-08-27T00:00:00Z", ""].join("\n");
    return new Response(request.method === "HEAD" ? null : body, {status: 200, headers: {"content-type": "text/plain; charset=utf-8", "cache-control": "public, max-age=3600", "x-content-type-options": "nosniff"}});
  }
