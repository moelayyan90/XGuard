...
function catalog(env) {
  ...
  return { name: "XGuard", version: "6.0.0", product: "Public-source outcomes for agents", capabilities: items,
...
function page(request, env, item = null) {
  ...
  const items = liveOutcomes(env);
  const onboarding = onboardingPage(url.pathname, items, url.searchParams.get("capability"));
  const title = item ? `${item.name} — XGuard` : onboarding.title || "XGuard — Three sources. One usable result.";
  const description = item?.description || onboarding.description || "Give your agent the pages or feeds. Get normalized evidence, comparable product offers or a clean news digest in one call.";
...
export async function handleOutcomeRoute(request, env, ctx) {
  ...
  if (read && url.pathname === "/v1/pricing") return json({ version: "6.0.0", capabilities: liveOutcomes(env).map(x => ({ id: x.id, ...x.pricing })), execution_url: `${API}/v1/execute`, quote_optional: `${API}/v1/pricing/quote`, payment: "x402-v2", first_result: { intent: "demo" } });
  ...
  if (read && ["/mcp", "/.well-known/mcp/server-card.json", "/.well-known/xguard-tools.json"].includes(url.pathname)) return json({ name: "XGuard — Governed API Gateway", version: "6.0.0",
    serverInfo: { name: "XGuard — Governed API Gateway", version: "6.0.0" }, authentication: { required: false, schemes: [] },
