...
test("Action Rail publishes the protocol-neutral execution contract", async () => {
  const response = await actionRail.fetch(new Request("https://xguardgate.com/api/v1/actions"), { ACTION_EXECUTION_CREDITS: "1" });
...
test("Action Rail pricing charges only successful execution", async () => {
  const response = await actionRail.fetch(new Request("https://xguardgate.com/api/v1/actions/pricing"), { ACTION_EXECUTION_CREDITS: "1" });
  const data = await response.json();
  assert.equal(data.credits_per_successful_execution, 1);
...
test("Action Rail blocks private execution targets before billing", async () => {
  const response = await actionRail.fetch(new Request("https://xguardgate.com/api/v1/actions/permits", {
    method: "POST",
...
test("Action Rail ignores unrelated paths", async () => {
  const response = await actionRail.fetch(new Request("https://xguardgate.com/api/not-an-action"), {});
