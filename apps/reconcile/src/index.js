...
HTTP 402 → pay $0.002 USDC on Base → retry with x402 payment header → JSON result</pre><p><a href="/docs">Machine-readable docs</a> · <a href="/llms.txt">llms.txt</a> · <a href="https://xguardgate.com">XGuard Relay</a></p></main></body></html>`, { headers: { "content-type": "text/html; charset=utf-8", "cache-control": "public, max-age=120", "content-security-policy": "default-src 'self'; style-src 'unsafe-inline'; frame-ancestors 'none'; base-uri 'none'", "x-frame-options": "DENY", "x-content-type-options": "nosniff" } });
}

function docs(env) {
  return json({ name: "XGuard Reconcile", version: VERSION, description: "Post-timeout settlement reconciliation for Base x402 EIP-3009 payments.", endpoint: "GET /v1/reconcile?from=<EVM_ADDRESS>&nonce=<BYTES32>", price_usd: 0.002, network: NETWORK, asset: BASE_USDC, scheme: "exact", facilitator: env.X402_FACILITATOR || "https://facilitator.xpay.sh", non_custodial: true, website: "https://xguardgate.com", llms: "https://reconcile.xguardgate.com/llms.txt" });
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/healthz" && request.method === "GET") return json({ status: "ok", service: "XGuard Reconcile", version: VERSION, payment_ready: isAddress(env.XGUARD_TREASURY_USDC_ADDRESS), network: NETWORK, price: PRICE });
